...
public class ReplaceFurniture implements BasicCommand {
    ...
    public void execute(CommandSourceStack commandSourceStack, String[] args) {
        ...
        if (args.length > 1) {
            ...
            if (!CustomFurniture.isInRegistry(args[0])) {
                player.sendMessage("<old furniture> doesn't exist.");
                return;
            }

            if (!CustomFurniture.isInRegistry(args[1])) {
                player.sendMessage("<new furniture> doesn't exist.");
                return;
            }
            ...
            player.sendMessage(MiniMessage.miniMessage().deserialize("<red>Command Syntax: /ReplaceFurnitureNear <old furniture> <new furniture>"));
        }
    }
    ...
    public Collection<String> suggest(CommandSourceStack commandSourceStack, String[] args) {
        if (args.length < 2) {
            return CustomFurniture.getNamespacedIdsInRegistry();
        }
